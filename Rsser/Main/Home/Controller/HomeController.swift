...
class HomeController: UIViewController {
    ...
    let fileProvider = CloudFileProvider(containerId: "iCloud.com.wannayoung.rsser")
...
extension HomeController: UITableViewDataSource, UITableViewDelegate, SwipeTableViewCellDelegate {
    ...
    func uploadToiCloud() {
        let realmPath = NSHomeDirectory()+"/Documents/default.realm"
        let realmData = try? Data(contentsOf: URL(fileURLWithPath: realmPath))
        fileProvider?.create(folder: "rss", at: "/", completionHandler: { error in
            guard error == nil else { return }
            self.fileProvider?.writeContents(path: "/rss/default.realm", contents: realmData, overwrite: true, completionHandler: { (error) in
                guard error == nil else { return }
    ...
    func downloadFromiCloud() {
        let realmPath = NSHomeDirectory()+"/Documents/default.realm"
        fileProvider?.contents(path: "/rss/default.realm", completionHandler: {
            contents, error in
            if let contents = contents {
                do {
                    try contents.write(to: URL(fileURLWithPath: realmPath))
